...
import { Search, Users, ShieldOff, Shield, ExternalLink } from 'lucide-react'
...
function UserRow({
  ...
  return (
    <tr className="border-b border-line transition-colors hover:bg-surface-alt">
      ...
      <td className="px-5 py-4">
        {user.total_transactions != null && user.total_transactions > 0 ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-warning-soft px-2.5 py-0.5 text-xs font-bold text-warning">
      ...
      <td className="px-5 py-4">
        <div className="flex items-center gap-2">
          <Link to={`/admin/pengguna/${user.id}`}>
            <Button variant="ghost" size="sm" className="gap-1 text-primary hover:bg-primary-soft">
      ...
      <div className="overflow-hidden rounded-2xl border border-line bg-surface">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line bg-surface-alt text-left">
              ...
              <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-muted">Transaksi</th>
            ...
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <tr key={i} className="border-b border-line">
                  ...
                  <td className="px-5 py-4"><Skeleton className="h-8 w-28 rounded-xl" /></td>
                </tr>
              ))
            ) : users.length === 0 ? (
              <tr>
        ...
        {lastPage > 1 && (
          <div className="flex items-center justify-between border-t border-line px-5 py-3">
            <span className="text-sm text-muted">Halaman {page} dari {lastPage} · {total} total</span>
