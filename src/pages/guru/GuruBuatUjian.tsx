...
import { ArrowLeft, Save, ClipboardList, Clock, BarChart2, FileText, BookOpen } from 'lucide-react'
...
type Step = 'info' | 'pengaturan' | 'preview'
    ...
    <div className="mx-auto max-w-2xl space-y-6">
        ...
        {STEPS.map((s, i) => (
          <div key={s.key} className="flex flex-1 items-center gap-2">
            <button
              ...
              disabled={i > stepIdx}
              className={cn(
                'flex flex-1 items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold transition-all',
                s.key === step
                  ? 'bg-primary text-white shadow-sm'
                  : i < stepIdx
                    ? 'bg-success-soft text-success cursor-pointer hover:bg-success/20'
            ...
            {i < STEPS.length - 1 && (
              <div className={cn('h-0.5 w-4 shrink-0 rounded-full', i < stepIdx ? 'bg-success' : 'bg-line')} />
            )}
          </div>
        ))}
      </div>
        ...
        <Card className="space-y-5">
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-muted">
              Nonaktif: ujian belum bisa dikerjakan siswa. Aktif: langsung bisa dikerjakan.
            </p>
          </div>
        </Card>
      )}
        ...
        <Card className="space-y-4">
          ...
          {form.instructions && (
            <div>
              ...
              <p className="rounded-xl border border-line bg-surface-alt px-4 py-3 text-sm text-ink whitespace-pre-line">
                {form.instructions}
              </p>
            </div>
          )}
      ...
      <div className="flex items-center gap-3">
        {step !== 'info' && (
          <Button
            variant="outline"
            onClick={() => setStep(step === 'preview' ? 'pengaturan' : 'info')}
          >
